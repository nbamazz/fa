...
public class CrateCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("\u00a76/crate create <id>\u00a77, setblock <id> <material>, addreward <id> <material|PRIME_PICKAXE|EXCAVATOR_PICKAXE> <amount> <weight> <enchants|none> <name...>, key <player> <keyId> <amount>, keyall <keyId> <amount>, open <id>, list, reload");
            return true;
        }
    ...
    private void handleAddReward(CommandSender sender, String[] args) {
        if (!requireAdmin(sender)) return;
        if (args.length < 7) {
            sender.sendMessage("\u00a7cUsage: /crate addreward <id> <material|PRIME_PICKAXE|EXCAVATOR_PICKAXE> <amount> <weight> <enchants|none> <name...>");
            sender.sendMessage("\u00a77Enchants format: sharpness:5,unbreaking:3  (use 'none' for no enchants)");
            return;
        }
        String id = args[1].toLowerCase();
        if (!plugin.getCrateManager().exists(id)) {
        ...
        String enchantsArg = args[5];
        String name = String.join(" ", List.of(args).subList(6, args.length));

        java.util.Map<String, Integer> enchants = new java.util.LinkedHashMap<>();
        if (!enchantsArg.equalsIgnoreCase("none")) {
            for (String part : enchantsArg.split(",")) {
                String[] kv = part.split(":");
                if (kv.length != 2) {
                    sender.sendMessage("\u00a7cBad enchant entry: " + part + " (use format name:level, e.g. sharpness:5)");
                    return;
                }
                String enchName = kv[0].trim().toLowerCase();
                org.bukkit.enchantments.Enchantment ench = org.bukkit.Registry.ENCHANTMENT.get(org.bukkit.NamespacedKey.minecraft(enchName));
                if (ench == null) {
                    sender.sendMessage("\u00a7cUnknown enchantment: " + enchName);
                    return;
                }
                int level = parseIntOr(kv[1].trim(), 1);
                enchants.put(enchName, level);
            }
        }

        Reward reward;
        if (typeArg.equals("PRIME_PICKAXE")) {
            reward = new Reward(name, Reward.Type.PRIME_PICKAXE, Material.DIAMOND_PICKAXE, 1, weight, enchants);
        } else if (typeArg.equals("EXCAVATOR_PICKAXE")) {
            reward = new Reward(name, Reward.Type.EXCAVATOR_PICKAXE, Material.NETHERITE_PICKAXE, 1, weight, enchants);
        } else {
            Material mat = Material.matchMaterial(typeArg);
            if (mat == null) { sender.sendMessage("\u00a7cUnknown material: " + typeArg); return; }
            reward = new Reward(name, Reward.Type.ITEM, mat, amount, weight, enchants);
        }
